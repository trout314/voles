...
Given the kernel $K$ and right-hand side $g$, solves for $y$.

**`solve_VIE_2`** — Type-2 Volterra integral equation:

$$y(t) = g(t) + \int_0^t K(t-s) y(s) \, ds$$

Given the kernel $K$ and forcing function $g$, solves for $y$.

**`solve_VIDE`** — Volterra integro-differential equation:

$$y'(t) = a(t) y(t) + g(t) + \int_0^t K(t-s) y(s) \, ds$$

Given the kernel $K$, coefficient $a$, and forcing function $g$, together with an initial value $y(0)$, solves for $y$.
